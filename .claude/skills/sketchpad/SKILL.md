...
5. Then use `sketchpad_show` with the same `turn_id` for what belongs on the iPad:
   ...
   - **Put words on the page** when they belong next to the drawing rather than in a panel to be read once — a list to tick off, a definition to keep in view, something to annotate. `text` plus `place_as_layer: true` and no `image_path`: it becomes a note they can move, resize, trace and draw over. Keep it short; a paragraph reads better in the panel, which scrolls.
