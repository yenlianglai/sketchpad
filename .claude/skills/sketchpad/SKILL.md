...
4. Do the work in this session as usual. Keep terminal narration to one line.
5. Answer with `sketchpad_show`, always with the same `turn_id`. Pick what actually helps:
   - **Text is the normal answer.** One or two sentences in `text`. Most turns need nothing else.
   - **Ask, when you would otherwise be guessing.** If the page could mean two things, say which two and ask — do not pick one and act on it. A question costs them five seconds; a wrong answer costs a round trip and a canvas to tidy up.
