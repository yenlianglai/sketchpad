...
describe('what the iPad app asks permission for', () => {
  // Info.plist is generated by xcodegen from project.yml, so editing the plist looks like it works
  // right up until someone regenerates. project.yml is the source of truth, and is what is checked.
  const projectYml = readFileSync(join(ROOT, 'ios', 'project.yml'), 'utf8')
  const appSource = readFileSync(join(ROOT, 'ios', 'Sketchpad', 'ServerConnection.swift'), 'utf8')
    + readFileSync(join(ROOT, 'ios', 'Sketchpad', 'PairingSheet.swift'), 'utf8')

  test('does not include a camera it no longer uses', () => {
    assert.ok(!projectYml.includes('NSCameraUsageDescription'), 'the QR scanner is gone; the permission should be too')
    assert.ok(!/AVCaptureDevice|AVCaptureSession/.test(appSource))
  })

  test('explains the local network, which it genuinely needs', () => {
    assert.match(projectYml, /NSLocalNetworkUsageDescription:/)
  })

  test('is written in the language the interface is in', () => {
    const descriptions = [...projectYml.matchAll(/NS\w*UsageDescription: (.+)/g)].map(m => m[1])
    assert.ok(descriptions.length > 0)
    for (const text of descriptions) {
      assert.ok(!/[一-鿿]/.test(text), `permission prompt is not in English: ${text}`)
    }
  })

  test('answers the encryption question up front', () => {
    assert.match(projectYml, /ITSAppUsesNonExemptEncryption:/)
  })

  test('and the generated plist is not committed, so it cannot swallow an edit', () => {
